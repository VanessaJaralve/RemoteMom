...
| Phase 3: Public Beta Readiness | More / Feedback | Fix Email Feedback action crash | Not Started |  | Smoke test failed: tapping Email Feedback shows a cannot-read-property error. Fix the feedback action so it opens email when available or shows a calm fallback when unavailable. |
